...
pub fn guide(topic: &str) -> Option<&'static str> {
    match topic.to_ascii_lowercase().as_str() {
        "core" | "rules" | "ante8" => Some(
            "Goal: clear Small, Big, and Boss blinds through Ante 8. Agent loop: game_status; query matching replays before each blind; get_decision; examine decision_checks.ordering when jokers are present; examine decision_checks.consumables for owned or shop Tarot/Planet/Spectral; examine decision_checks.shop and decision_checks.slots during SHOP state; lookup unknown effects; take one legal action with current decision_id; refresh and retry when stale_decision is returned; use play_selected or discard_selected with 1-based card_indices for arbitrary hand positions; when legal_actions_truncated is true, call get_decision with action_offset=legal_actions_next_offset and action_limit, repeating until next_offset is null; the same rule applies with action_type filters. observe is read-only and does not return actions, while wait_for_state only confirms state and returns next_step=get_decision; call get_decision after either. In ROUND_EVAL use proceed_round to enter the shop, then next_round to continue from the shop. In GAME_OVER, from_game_over is a ui_click and return_to_menu is also a safe_transition. Never infer face-down cards.",
        ),
        "hands" | "scoring" => Some(
            "Poker hands score Chips multiplied by Mult. Planet consumables level a named hand. Use the hand_values tool for the live poker-hand contract; controller scores are estimates unless explicitly exact. score_hand card_indices and score_analysis.scoring_cards use 1-based hand positions, matching play/discard actions; omitted indices use the live highlight or best five-card subset. score_analysis.hand_key describes the cards scored for that call, score_analysis.score_scope identifies current-hand versus selected-card scope, and score_analysis.run_chips plus score_analysis.blind_chips_remaining expose cumulative run progress. run_info.cards_played is cumulative by rank across the run; run_info.round_scores.cards_played.amt is the current round's count.",
        ),
